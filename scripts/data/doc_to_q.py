...
def load_documents(args: argparse.Namespace) -> Dict[str, Document]:
    ...
    logger.info(f"Number of sources: {len(source_to_doc)}")
    return source_to_doc


def init_prompting_model(args: argparse.Namespace):
    prompting_model: BaseQAModel
    if args.prompt_model in ['gpt-3.5-turbo', 'gpt-4', 'gpt-4-turbo']:
...
def create_positive_n_negative_examples(args: argparse.Namespace, filter_fn: Callable):
    ...
    documents_dataset = []
    all_sources = list(source_to_doc.keys())
    # hard negatives are document chunks from the SAME source
    for source in all_sources:
        # first we consider all possible positive pairs
        unique_documents_per_source = _get_unique_documents(source_to_doc[source])
        if len(unique_documents_per_source) < num_hard_negs_per_doc + 1:
            continue
        ...
        for j, pos_doc in enumerate(unique_documents_per_source):
            ...
            hard_neg_docs = random.sample(unique_documents_per_source[:j] + unique_documents_per_source[j+1:], num_hard_negs_per_doc)
    ...
    ### save
    save_path = os.path.join(args.save_dir, "all_doc2q.pkl")
...
def _batch_generate_questions(prompting_model: BaseQAModel, documents: List[Document], prompt, verbose=False):
    ...
    for o in output:
        ...
        extracted_questions.add(questions)
    return list(extracted_questions)
...
def _batch_generate_questions_from_dataset(prompting_model: BaseQAModel, documents_dataset, prompt, batch_size=8, num_data=1000):
    ...
    for i, batched_samples in enumerate(iterator):
        questions = _batch_generate_questions(
            prompting_model,
            [s['pos_doc'] for s in batched_samples],
            prompt = prompt,
            verbose = True
        )
...
def create_heldout_test_dset(args, doc2q_prompt):
    ...
    gather_n_save_documents(eval_dataset, "eval_documents.pkl")
    gather_n_save_documents(test_dataset, "test_documents.pkl")
    return eval_dataset, test_dataset


def eval_dataset_unique_sources(eval_dataset):
    new_eval_dataset = []
    seen_sources = set()
    for sample in eval_dataset:
        source = sample['pos_doc'].metadata['source']
        if source in seen_sources:
            continue
        seen_sources.add(source)
        new_eval_dataset.append(sample)
...
def create_train_dset(args, doc2q_prompt):
    ...
    with open(os.path.join(args.save_dir, "all_doc2q.pkl"), "rb") as fread:
        documents_dataset = pickle.load(fread)
    logger.info(f"Loaded all q2docs pairs: {len(documents_dataset)}")
    ...
    for sample in held_out_documents_dataset:
        pos_doc = sample['pos_doc']
        doc_id = _hash_document(pos_doc)
    ...
    for sample in potential_train_dataset:
        ...
        if doc_id not in eval_n_test_doc2q_mapping:
            ...
            ### add only if there are new questions, if document is the same
