...
from open_rqa.trainers.retriever.arguments import ModelArguments, DataArguments, ReplugTrainingArgs, RetrievalQATrainingArguments
...
if __name__ == "__main__":
	parser = HfArgumentParser(
		dataclass_types=(ModelArguments, DataArguments, ReplugTrainingArgs, RetrievalQATrainingArguments),
		description="QA Retriever training script"
	)
	if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
		# If we pass only one argument to the script and it's the path to a json file,
		# let's parse it to get our arguments.
		model_args, data_args, replug_args, training_args = parser.parse_json_file(
			json_file=os.path.abspath(sys.argv[1])
		)
	else:
		model_args, data_args, replug_args, training_args = parser.parse_args_into_dataclasses()
	...
	search_kwargs = SEARCH_CONFIG[replug_args.search_algo]
